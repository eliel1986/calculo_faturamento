...
function calculo() {
	...
	const volOrig = document.getElementById('vol_orig').value;
	const pesoDest = document.getElementById('peso_dest').value;

	if (densDest !== '' && pesoDest !== '' && densOrig !== '' && volOrig !== '') { //validação do preenchimento
		const vol_orig_peso = (volOrig * densOrig);

		document.getElementById('peso_orig').value = `${vol_orig_peso} kilos`;		
		...
		const difPesos = (pesoDest - vol_orig_peso).toFixed(0);
